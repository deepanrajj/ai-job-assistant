...
- Prefer `val` over `var`, except for editable JPA entity columns; see
  section 4.
- Avoid `!!`.
...
- Ids are assigned by the service, not generated by the database. A new
  entity extends `AssignedIdEntity` in `com.smartjobtracker.persistence`,
  which owns the `@Id`; do not declare another one on the subclass.
- Update an entity by mutating the instance the repository returned and
  letting dirty checking write it. Do not rebuild an entity to change a
  field: the rebuilt object reports itself as new, and the save is
  rejected rather than silently overwriting the row.
- Make a column `var` when something edits it, and leave it `val`
  otherwise. This is the one place where `var` is preferred over `val`.
- Changing identifier generation, or adding `@Version` and optimistic
  locking, is an architecture decision. Raise it rather than doing it
  inside a feature task. Reasoning and rejected alternatives are in
