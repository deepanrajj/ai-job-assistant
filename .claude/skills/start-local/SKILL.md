...
| Backend pod crash-loops and never becomes ready | PostgreSQL is failing, **or** the secret was recreated after the database was initialised | check the postgres pod first, then `kubectl logs deployment/smart-job-tracker-backend -n smart-job-tracker --previous`. `password authentication failed` means the second cause: the password is fixed at `initdb` and the claim outlives the pod, so restarting cannot fix it. The database has to be reset, and the claim does not reinitialise on its own: follow `docs/setup.md` "Resetting The Kubernetes Database" in order - deleting the PVC while postgres runs hangs, and a `rollout restart` cannot recreate it |
...
- `npm run dev:compose` is a third runtime, and the only one that
  ...
  not come up at all, or when the database needs to survive a restart.
