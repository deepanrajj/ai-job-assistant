...
Every `id` above is a UUID assigned by the service, not generated by the
database. Entities therefore extend `AssignedIdEntity`, a
`@MappedSuperclass` in `com.smartjobtracker.persistence` that owns the
`@Id` and implements Spring Data's `Persistable` so a save inserts
without a preceding select and returns the instance it was given.

Two rules follow, and both matter when adding an entity:

- A new entity extends `AssignedIdEntity` and does not declare its own
  `@Id`.
- An update mutates the instance the repository returned. Rebuilding an
  entity to change a field makes it report itself as new, and the save
  is rejected rather than silently overwriting the row.

Changing identifier generation, or adding `@Version` and optimistic
locking, is an architecture decision rather than a task-level one. See
`business/bug-001-assigned-id-entities-merge-on-save-plan.md` for the
