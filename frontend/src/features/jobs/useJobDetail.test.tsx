...
interface IJobDetailProbeProps {
  jobId?: string;
}

const JobDetailProbe = ({ jobId = MOCK_JOB_IDS.celonis }: IJobDetailProbeProps) => {
  ...
  return (
    <p>
      {job?.company} description={String(job?.description)} notes={job?.notes.length}
    </p>
  );
};
...
describe('useJobDetail', () => {
  it('loads the job for the route id and widens it to the detail shape', async () => {
    server.use(
    ...
    // `description: null` on the wire becomes `undefined` through the
    // response mapper and then `''` through the detail widening, and the
    // detail collections start empty because no endpoint carries them.
    expect(await screen.findByText(/Celonis/)).toHaveTextContent('Celonis description= notes=0');
  });
  ...
  it('reports an id that is not a uuid as not found', async () => {
    ...
    render(<JobDetailProbe jobId="job-001" />);
  ...
  it('loads the new job when the route id changes', async () => {
    ...
    const { rerender } = render(<JobDetailProbe />);

    expect(await screen.findByText(/Celonis/)).toBeInTheDocument();

    rerender(<JobDetailProbe jobId={MOCK_JOB_IDS.miro} />);
